 ...
 */
class UUV2PosePlugin : public plugin::PluginBase
{
public:
        UUV2PosePlugin() : PluginBase(),
                mp_nh("~UUV2Pose")
        { }
        ...
        void initialize(UAS &uas_)
        {
                ...
                uuv2_pose_sub = mp_nh.subscribe("pose", 1, &UUV2PosePlugin::uuv2_pose_cb, this);
...
private:
        ...
        void uuv2_pose_send
                (uint64_t usec,
                        ...
                        Eigen::Vector3d &v)
        {
                mavlink::common::msg::UUV_TWO_POS pos;
        ...
        /* -*- mid-level helpers -*- */
        void uuv2_pose_cb(const geometry_msgs::PoseStamped::ConstPtr &pose)
        {
                ...
                uuv2_pose_send(pose->header.stamp.toNSec() / 1000,
                                q,
                                position);
        }
...
PLUGINLIB_EXPORT_CLASS(mavros::extra_plugins::UUV2PosePlugin, mavros::plugin::PluginBase)
